...
const getRandomWord = (lang, difficulty, pack) => {
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ username, room, lang, difficulty }) => {
    socket.join(room);

    if (!games[room]) {
      games[room] = {
        players: {},
        word: getRandomWord(lang, difficulty),
        lang,
        difficulty,
        startTime: Date.now(),
      };
    ...
    games[room].players[socket.id] = { username, score: 0, guess: '', attempts: [] };
    io.to(room).emit('gameUpdate', games[room]);
    io.emit('roomsList', Object.keys(games));
  });
  ...
  socket.on('makeGuess', ({ guess, room }) => {
    const game = games[room];
    if (game) {
      const player = game.players[socket.id];
      player.guess = guess;
      player.attempts.push(guess.toUpperCase());

      if (guess.toUpperCase() === game.word) {
        player.score += 10;
        game.word = getRandomWord(game.lang, game.difficulty); // Use appropriate difficulty
      }
...
app.get('/word', (req, res) => {
  ...
  const word = getRandomWord(lang, difficulty, pack);
  if (!word) {
...
//     socket.on('joinGame', ({ username, lang, difficulty, pack }) => {
//       if (!games[lang]) {
//         games[lang] = {
//           players: {},
//           word: getRandomWord(lang, difficulty, pack),
//           startTime: Date.now(),
//         };
//       }

//       if (Object.keys(games[lang].players).length >= 30) {
//         socket.emit('roomFull', 'This game room is full.');
//         return;
//       }

//       games[lang].players[socket.id] = { username, score: 0, guess: '', attempts: [] };
//       socket.join(lang);
//       io.to(lang).emit('gameUpdate', games[lang]);
//     });
...
//       if (game && guess.toUpperCase() === game.word) {
//         game.players[socket.id].score += 1;
//         game.players[socket.id].attempts.push(guess.toUpperCase());
//         game.word = getRandomWord(lang, difficulty, pack);
//         io.to(lang).emit('gameUpdate', game);
//       } else {
//         game.players[socket.id].attempts.push(guess.toUpperCase());
//         socket.emit('guessFeedback', 'Incorrect guess. Try again!');
...
//     socket.on('disconnect', () => {
//       for (const lang in games) {
//         if (games[lang].players[socket.id]) {
//           delete games[lang].players[socket.id];
//           io.to(lang).emit('gameUpdate', games[lang]);
//         }
//       }
